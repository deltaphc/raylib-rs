    ...
    main()
{
    ...
    //----------------------------------------------------------------------------------
    Vector2 windowOffset = {screen_width / 2 - 200 / 2, screen_height / 2 - 465 / 2};
    ...
    return Box::new(move |rl: &mut RaylibHandle, thread: &RaylibThread| -> () // Detect window close button or ESC key
    {
        ...
        if (imageLoaded)
            imageScale += (float)GetMouseWheelMove(); // Image scale control
        //----------------------------------------------------------------------------------
        ...
        if (texture.id != 0)
        {
            ...
            d.draw_text(FormatText("SCALE x%.0", imageScale), 20, screen_height - 40, 20, GetColor(GuiGetStyle(DEFAULT, LINE_COLOR)));
        }
        else
        ...
        if (importWindowActive)
        {
            ...
            if (GuiValueBox((Rectangle){windowOffset.x + 60, windowOffset.y + 100, 80, 25}, NULL, &widthValue, 0, 8192, widthEditMode))
            ...
            if (GuiValueBox((Rectangle){windowOffset.x + 60, windowOffset.y + 130, 80, 25}, NULL, &heightValue, 0, 8192, heightEditMode))
            ...
            if (GuiValueBox((Rectangle){windowOffset.x + 55, windowOffset.y + 370, 85, 25}, NULL, &headerSizeValue, 0, 10000, headerSizeEditMode))
