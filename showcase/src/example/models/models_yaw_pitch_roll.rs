...
pub fn run(rl: &mut RaylibHandle, thread: &RaylibThread) -> crate::SampleOut {
    ...
    let tex_angle_gauge = rl
        .load_texture(&thread, "original/models/resources/angle_gauge.png")
        .unwrap();
    let tex_background = rl
        .load_texture(&thread, "original/models/resources/background.png")
        .unwrap();
    let tex_pitch = rl
        .load_texture(&thread, "original/models/resources/pitch.png")
        .unwrap();
    let tex_plane = rl
        .load_texture(&thread, "original/models/resources/plane.png")
        .unwrap();

    let mut framebuffer = rl.load_render_texture(&thread, 192, 192).unwrap();

    let mut model = rl
        .load_model(&thread, "original/models/resources/plane.obj")
        .unwrap();
    {
        ...
        let texture = unsafe {
            let mut t = rl
                .load_texture(&thread, "original/models/resources/plane_diffuse.png")
                .unwrap();
