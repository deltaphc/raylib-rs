...
impl RaylibHandle {
    ...
    /// Sets minimum window dimensions (for `rl::ConfigFlag::FLAG_WINDOW_RESIZABLE`).
    #[inline]
    ...
    pub fn set_mouse_position(&self, position: impl Into<Vector2>) {
        unsafe {
            ...
            rl::SetMousePosition(x as i32, y as i32);
        }
    }
    ...
    pub fn update_camera(&self, camera: &mut Camera3D) {
        unsafe {
            let mut fficam: rl::Camera3D = (*camera).into();
            rl::UpdateCamera(&mut fficam);
            *camera = fficam.into();
        }
    }
    ...
    pub fn load_font_data(
        ...
        unsafe {
            let ci_arr_ptr = match chars {
                ...
                None => {
                    rl::LoadFontData(c_filename.as_ptr(), font_size, std::ptr::null_mut(), 0, sdf)
                }
            };
    ...
    pub fn update_vr_tracking(&self, camera: &mut Camera3D) {
        unsafe {
            let mut fficam: rl::Camera3D = (*camera).into();
            rl::UpdateVrTracking(&mut fficam);
            *camera = fficam.into();
        }
    }
