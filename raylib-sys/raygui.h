 ...
 *       Includes ricons.h header defining a set of 128 icons (binary format) to
 *be used on multiple controls and following raygui styles
 *
 *   #define RAYGUI_TEXTBOX_EXTENDED
 *       Enables advance GuiTextBox()implementation with text selection and
...
#define RAYGUIDEF                                                              \
  __declspec(dllexport) extern // We are building raygui as a Win32 shared
                               // library (.dll).
...
#ifdef __cplusplus
#define RAYGUIDEF                                                              \
  extern "C" // Functions visible from other files (no name mangling of
             // functions in C++)
#else
...
RAYGUIDEF Color GuiColorPicker(Rectangle bounds,
                               Color color); // Color Picker control

// Styles loading functions
RAYGUIDEF void GuiLoadStyle(const char *fileName); // Load style file (.rgs)
RAYGUIDEF void GuiLoadStyleProps(const int *props,
                                 int count); // Load style properties from array
RAYGUIDEF void
GuiLoadStyleDefault(void); // Load style default over global style
RAYGUIDEF void GuiUpdateStyleComplete(
    void); // Updates full style properties set with default values
...
static GuiTextBoxState guiTextBoxState = {
    .cursor = -1,
    .start = 0,
    .index = 0,
    .select = -1}; // Keeps state of the active textbox
#endif
...
static int
GetKeyPressed(void); // -- GuiTextBox(), GuiTextBoxMulti(), GuiValueBox()
//-------------------------------------------------------------------------------
...
static void DrawRectangleRec(Rectangle rec,
                             Color color); // Draw rectangle filled with color
static void DrawRectangleLinesEx(Rectangle rec, int lineThick,
                                 Color color); // Draw rectangle outlines
static void
...
static int GetTextWidth(const char *text) // TODO: GetTextSize()
{
  ...
  if ((text != NULL) && (text[0] != '\0'))
    size = MeasureTextEx(guiFont, text, GuiGetStyle(DEFAULT, TEXT_SIZE),
    ...
    if (iconId > 0) {
      ...
      DrawIcon(iconId,
               RAYGUI_CLITERAL(Vector2){
                   position.x, bounds.y + bounds.height / 2 - RICONS_SIZE / 2 +
                                   VALIGN_OFFSET(bounds.height)},
               1, tint);
      position.x += (RICONS_SIZE + ICON_TEXT_PADDING);
    }
#endif
    DrawTextEx(guiFont, text, position, GuiGetStyle(DEFAULT, TEXT_SIZE),
               GuiGetStyle(DEFAULT, TEXT_SPACING), tint);
    //---------------------------------------------------------------------------------
  }
}
...
// Get custom gui font
RAYGUIDEF Font GuiGetFont(void) { return guiFont; }
...
#define WINDOW_STATUSBAR_HEIGHT 22
  ...
  if (bounds.height < statusBarHeight * 2)
    bounds.height = statusBarHeight * 2;
  ...
  DrawRectangleLinesEx(
      ...
      Fade(GetColor(GuiGetStyle(DEFAULT, BORDER + (state * 3))), guiAlpha));
  DrawRectangleRec(
      RAYGUI_CLITERAL(Rectangle){
          bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH),
          bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH),
          bounds.width - GuiGetStyle(DEFAULT, BORDER_WIDTH) * 2,
          bounds.height - GuiGetStyle(DEFAULT, BORDER_WIDTH) * 2},
...
#define GROUPBOX_TEXT_PADDING 10
  ...
  // Draw control
  //--------------------------------------------------------------------
  DrawRectangle(bounds.x, bounds.y, GROUPBOX_LINE_THICK, bounds.height,
                Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                                       ? BORDER_COLOR_DISABLED
                                                       : LINE_COLOR)),
                     guiAlpha));
  DrawRectangle(bounds.x, bounds.y + bounds.height - 1, bounds.width,
                GROUPBOX_LINE_THICK,
                Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                                       ? BORDER_COLOR_DISABLED
                                                       : LINE_COLOR)),
                     guiAlpha));
  DrawRectangle(bounds.x + bounds.width - 1, bounds.y, GROUPBOX_LINE_THICK,
                bounds.height,
                Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                                       ? BORDER_COLOR_DISABLED
                                                       : LINE_COLOR)),
...
#define LINE_TEXT_PADDING 10
  ...
  //--------------------------------------------------------------------
  if (text == NULL)
    DrawRectangle(bounds.x, bounds.y + bounds.height / 2, bounds.width, 1,
                  color);
  else {
    ...
    DrawRectangle(bounds.x + LINE_TEXT_PADDING + textBounds.width + 4, bounds.y,
...
#define PANEL_BORDER_WIDTH 1
  ...
  //--------------------------------------------------------------------
  DrawRectangleRec(
      bounds, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                                     ? BASE_COLOR_DISABLED
                                                     : BACKGROUND_COLOR)),
                   guiAlpha));
  DrawRectangleLinesEx(
...
RAYGUIDEF Rectangle GuiScrollPanel(Rectangle bounds, Rectangle content,
  ...
  const int horizontalMin =
      hasHorizontalScrollBar
          ...
          : ((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
                 ...
                 : 0) -
                GuiGetStyle(DEFAULT, BORDER_WIDTH);
  const int horizontalMax =
      hasHorizontalScrollBar
          ? content.width - bounds.width + verticalScrollBarWidth +
                GuiGetStyle(DEFAULT, BORDER_WIDTH) -
                ((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
                     ? verticalScrollBarWidth
                     : 0)
          : -GuiGetStyle(DEFAULT, BORDER_WIDTH);
  const int verticalMin = hasVerticalScrollBar
                              ? -GuiGetStyle(DEFAULT, BORDER_WIDTH)
                              : -GuiGetStyle(DEFAULT, BORDER_WIDTH);
  const int verticalMax = hasVerticalScrollBar
    ...
    GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE,
                ((bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) -
                 ...
                 content.width) *
                    (bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) -
                     verticalScrollBarWidth));
    scrollPos.x = -GuiScrollBar(horizontalScrollBar, -scrollPos.x,
    ...
    GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE,
                ((bounds.height - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) -
                 ...
                 content.height) *
                    (bounds.height - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) -
                     horizontalScrollBarWidth));
    scrollPos.y = -GuiScrollBar(verticalScrollBar, -scrollPos.y, verticalMin,
                                verticalMax);
  }
  ...
  DrawRectangleLinesEx(
      bounds, GuiGetStyle(DEFAULT, BORDER_WIDTH),
      Fade(GetColor(GuiGetStyle(LISTVIEW, (float)BORDER + (state * 3))),
           guiAlpha));
  //--------------------------------------------------------------------
...
RAYGUIDEF bool GuiLabelButton(Rectangle bounds, const char *text) {
  ...
  int textWidth = MeasureTextEx(guiFont, text, GuiGetStyle(DEFAULT, TEXT_SIZE),
                                GuiGetStyle(DEFAULT, TEXT_SPACING))
                      .x;
...
RAYGUIDEF bool GuiImageButton(Rectangle bounds, const char *text,
                              Texture2D texture) {
...
// Image button control, returns true when clicked
RAYGUIDEF bool GuiImageButtonEx(Rectangle bounds, const char *text,
                                Texture2D texture, Rectangle texSource) {
  GuiControlState state = guiState;
  bool clicked = false;
...
RAYGUIDEF int GuiComboBox(Rectangle bounds, const char *text, int active) {
  ...
  DrawRectangle(
      bounds.x + GuiGetStyle(COMBOBOX, BORDER_WIDTH),
      bounds.y + GuiGetStyle(COMBOBOX, BORDER_WIDTH),
      bounds.width - 2 * GuiGetStyle(COMBOBOX, BORDER_WIDTH),
      bounds.height - 2 * GuiGetStyle(COMBOBOX, BORDER_WIDTH),
...
RAYGUIDEF bool GuiDropdownBox(Rectangle bounds, const char *text, int *active,
                              bool editMode) {
  GuiControlState state = guiState;
  ...
  // GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 2, 10,
  // 10 },
...
RAYGUIDEF void GuiTextBoxCopy(const char *text) {
  if ((text != NULL) && (guiTextBoxState.select != -1) &&
    ...
    // FIXME: `TextSubtext()` only lets use copy MAX_TEXT_BUFFER_LENGTH (1024)
    // bytes maybe modify `SetClipboardText()` so we can use it only on part of
    // a string
...
RAYGUIDEF void GuiTextBoxCut(char *text) {
  if ((text != NULL) && (guiTextBoxState.select != -1) &&
    ...
    // FIXME: `TextSubtext()` only lets use copy MAX_TEXT_BUFFER_LENGTH (1024)
    // bytes maybe modify `SetClipboardText()` so we can use it only on parts of
    // a string
...
RAYGUIDEF bool GuiTextBox(Rectangle bounds, char *text, int textSize,
                          bool editMode) {
    ...
    if (editMode) {
      ...
      if (active) {
        ...
        if (IsKeyPressed(KEY_RIGHT) ||
            (IsKeyDown(KEY_RIGHT) &&
             (framesCounter % GUI_TEXTBOX_CURSOR_SPEED_MODIFIER == 0))) {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds)) {
          if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            if ((framesCounter % GUI_TEXTBOX_CURSOR_SPEED_MODIFIER) == 0 &&
                move) {
              if (cursor == 0)
                MoveTextBoxCursorLeft(text);
...
RAYGUIDEF bool GuiSpinner(Rectangle bounds, const char *text, int *value,
...
RAYGUIDEF bool GuiValueBox(Rectangle bounds, const char *text, int *value,
                           int minValue, int maxValue, bool editMode) {
#define VALUEBOX_MAX_CHARS 32
  ...
  if (state == GUI_STATE_PRESSED) {
    DrawRectangle(
        bounds.x + GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        bounds.y + GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        bounds.width - 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        bounds.height - 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        Fade(GetColor(GuiGetStyle(VALUEBOX, BASE_COLOR_PRESSED)), guiAlpha));

    // Draw blinking cursor
    // NOTE: ValueBox internal text is always centered
    if (editMode && ((framesCounter / 20) % 2 == 0))
      DrawRectangle(bounds.x + GetTextWidth(textValue) / 2 + bounds.width / 2 +
                        2,
                    bounds.y + 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH), 1,
                    bounds.height - 4 * GuiGetStyle(VALUEBOX, BORDER_WIDTH),
                    Fade(GetColor(GuiGetStyle(VALUEBOX, BORDER_COLOR_PRESSED)),
                         guiAlpha));
  } else if (state == GUI_STATE_DISABLED) {
    DrawRectangle(
        bounds.x + GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        ...
        bounds.width - 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        bounds.height - 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH),
        Fade(GetColor(GuiGetStyle(VALUEBOX, BASE_COLOR_DISABLED)), guiAlpha));
  }
...
RAYGUIDEF bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize,
                               bool editMode) {
  static int framesCounter = 0; // Required for blinking cursor
  ...
  if ((state != GUI_STATE_DISABLED) && !guiLocked) {
    ...
    if (editMode) {
      ...
      if (keyCount < (textSize - 1)) {
        ...
        if (MeasureTextEx(guiFont, text, GuiGetStyle(DEFAULT, TEXT_SIZE), 1).y <
            (maxHeight - GuiGetStyle(DEFAULT, TEXT_SIZE))) {
          if (IsKeyPressed(KEY_ENTER)) {
            text[keyCount] = '\n';
            keyCount++;
  ...
  if (state == GUI_STATE_PRESSED) {
    ...
    if (editMode && ((framesCounter / 20) % 2 == 0)) {
      ...
      Rectangle cursor = {bounds.x + GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
                              GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING) +
                              GetTextWidth(line),
                          bounds.y + GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
  ...
  DrawTextRec(
      guiFont, text, textAreaBounds, GuiGetStyle(DEFAULT, TEXT_SIZE),
      GuiGetStyle(DEFAULT, TEXT_SPACING), true,
      Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state * 3))), guiAlpha));
  //--------------------------------------------------------------------
...
RAYGUIDEF float GuiSliderPro(Rectangle bounds, const char *textLeft,
  ...
  DrawRectangle(bounds.x + GuiGetStyle(SLIDER, BORDER_WIDTH),
                bounds.y + GuiGetStyle(SLIDER, BORDER_WIDTH),
                bounds.width - 2 * GuiGetStyle(SLIDER, BORDER_WIDTH),
                bounds.height - 2 * GuiGetStyle(SLIDER, BORDER_WIDTH),
...
RAYGUIDEF float GuiSlider(Rectangle bounds, const char *textLeft,
                          const char *textRight, float value, float minValue,
                          float maxValue) {
...
RAYGUIDEF float GuiSliderBar(Rectangle bounds, const char *textLeft,
                             const char *textRight, float value, float minValue,
                             float maxValue) {
  return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue,
                      0);
...
RAYGUIDEF float GuiProgressBar(Rectangle bounds, const char *textLeft,
  ...
  if (state != GUI_STATE_DISABLED)
    progress.width = (int)(value / (maxValue - minValue) *
                           (float)(bounds.width -
                                   2 * GuiGetStyle(PROGRESSBAR, BORDER_WIDTH)));
  //--------------------------------------------------------------------
...
RAYGUIDEF void GuiStatusBar(Rectangle bounds, const char *text) {
  ...
  DrawRectangleRec(
      RAYGUI_CLITERAL(Rectangle){
          bounds.x + GuiGetStyle(STATUSBAR, BORDER_WIDTH),
          bounds.y + GuiGetStyle(STATUSBAR, BORDER_WIDTH),
          bounds.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) * 2,
          bounds.height - GuiGetStyle(STATUSBAR, BORDER_WIDTH) * 2},
      Fade(GetColor(GuiGetStyle(STATUSBAR, (state != GUI_STATE_DISABLED)
                                               ? BASE_COLOR_NORMAL
                                               : BASE_COLOR_DISABLED)),
           guiAlpha));
      ...
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        if (!isVertical) {
          ...
          if (CheckCollisionPointRec(mousePoint, scrollArea))
            value = ((float)(mousePoint.x - scrollArea.x - slider.width / 2) *
                     range) /
          ...
          Rectangle scrollArea = {
              ...
              scrollbar.height};
          if (CheckCollisionPointRec(mousePoint, scrollArea))
            value = ((float)(mousePoint.y - scrollArea.y - slider.height / 2) *
                     range) /
...
// List View control
RAYGUIDEF int GuiListView(Rectangle bounds, const char *text, int *scrollIndex,
                          int active) {
  int itemsCount = 0;
  const char **items = GuiTextSplit(text, &itemsCount, NULL);
...
RAYGUIDEF float GuiColorBarHue(Rectangle bounds, float hue) {
  ...
  if (state != GUI_STATE_DISABLED) {
    ...
    DrawRectangleGradientV(
        bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.y + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha));
    DrawRectangleGradientV(
        bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.y + (int)bounds.height / 6 +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.y + 2 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.y + 3 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.y + 4 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.y + 5 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6 -
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
...
RAYGUIDEF Color GuiColorPicker(Rectangle bounds, Color color) {
  ...
  Vector3 rgb = ConvertHSVtoRGB(hsv);
  color = RAYGUI_CLITERAL(Color){(unsigned char)(rgb.x * 255.0f),
                                 (unsigned char)(rgb.y * 255.0f),
                                 (unsigned char)(rgb.z * 255.0f), color.a};

  return color;
}

// Message Box control
RAYGUIDEF int GuiMessageBox(Rectangle bounds, const char *title,
...
#define MESSAGEBOX_BUTTON_PADDING 10
  ...
  Vector2 textSize =
      MeasureTextEx(guiFont, message, GuiGetStyle(DEFAULT, TEXT_SIZE), 1);
...
RAYGUIDEF int GuiTextInputBox(Rectangle bounds, const char *title,
                              const char *message, const char *buttons,
                              char *text) {
...
#define MAX_FILENAME_LENGTH 256
  ...
  int messageInputHeight = bounds.height - WINDOW_STATUSBAR_HEIGHT -
  ...
  if (GuiTextBox(textBoxBounds, text, MAX_FILENAME_LENGTH, textEditMode))
...
#define GRID_COLOR_ALPHA 0.15f // Grid lines alpha amount
  ...
  case GUI_STATE_NORMAL: {
    ...
    for (int i = 0; i < linesV; i++) {
      DrawRectangleRec(RAYGUI_CLITERAL(Rectangle){bounds.x + spacing * i,
    ...
    for (int i = 0; i < linesH; i++) {
      DrawRectangleRec(RAYGUI_CLITERAL(Rectangle){bounds.x,
                                                  bounds.y + spacing * i,
                                                  bounds.width, 1},
                       ((i % subdivs) == 0)
                           ? Fade(GetColor(GuiGetStyle(DEFAULT, LINE_COLOR)),
      ...
      while (!feof(rgsFile)) {
        ...
        case 'p': {
          ...
          if (controlId == 0) // DEFAULT control
          {
            ...
            // controls, NOTE: All DEFAULT properties should be defined first in
            // the file
...
RAYGUIDEF void GuiLoadStyleProps(const int *props, int count) {
  ...
  for (int k = 0; k < uncompleteSetProps; k++)
    GuiSetStyle(
        completeSets, k,
        props[completeSets * (NUM_PROPS_DEFAULT + NUM_PROPS_EXTENDED) + k]);
}
...
RAYGUIDEF void GuiLoadStyleDefault(void) {
  ...
  // Initialize default control-specific properties: BORDER_WIDTH, TEXT_PADDING,
  // TEXT_ALIGNMENT NOTE: Those properties are in default list but require
  // specific values by control type
...
RAYGUIDEF void GuiUpdateStyleComplete(void) {
  ...
  // NOTE: Some default properties depend on control type: BORDER_WIDTH,
  // TEXT_PADDING, TEXT_ALIGNMENT
...
static Color Fade(Color color, float alpha) {
  ...
  return RAYGUI_CLITERAL(Color){color.r, color.g, color.b,
...
static void DrawRectangleLinesEx(Rectangle rec, int lineThick, Color color) {
  ...
  DrawRectangle(rec.x, rec.y + lineThick, lineThick, rec.height - 2 * lineThick,
                color);
  DrawRectangle(rec.x + rec.width - lineThick, rec.y + lineThick, lineThick,
                rec.height - 2 * lineThick, color);
  DrawRectangle(rec.x, rec.y + rec.height - lineThick, rec.width, lineThick,
                color);
}
