    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            if (textAlignment == TEXT_ALIGN_CENTER)
                ...
                cursor.x = bounds.x + bounds.width - GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING) - GuiGetStyle(TEXTBOX, BORDER_WIDTH);
        }
        else
        {
            if (CheckCollisionPointRec(mousePoint, bounds))
            ...
            if (textLength > 0)
            {
                if (IsKeyPressed(KEY_BACKSPACE))
                {
                    ...
                    else
                    {
                        ...
                        int charUTF8Length = 0;
                        while ((charUTF8Length < textLength) && ((unsigned char)text[textLength - 1 - charUTF8Length] & 0b01000000) == 0)
                            charUTF8Length++;
